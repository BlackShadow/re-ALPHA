...
****/
//
// studiomdl.cpp: Generates a studio .mdl file from a .qc script (models/<scriptname>.mdl).
//
...
static int default_flip_triangles = 0;
...
void SimplifyModel(void)
{
	...
	// HL Alpha uses a single embedded animation block.
	for (i = 0; i < numseq; i++)
	{
...
void Build_Reference(s_model_t* pmodel)
{
	...
	for (i = 0; i < pmodel->numbones; i++)
	{
		...
		// convert to degrees
		// Alpha runtime quaternion decode uses a different Euler component order
		// than retail tools. Remap SMD angles so mesh->bone-space conversion
		// matches the engine's final bone transform.
		angle[0] = pmodel->skeleton[i].rot[2] * (180.0 / Q_PI);
		angle[1] = pmodel->skeleton[i].rot[0] * (180.0 / Q_PI);
		angle[2] = pmodel->skeleton[i].rot[1] * (180.0 / Q_PI);
...
void Option_Studio()
{
	...
	flip_triangles = default_flip_triangles;
...
void Cmd_Origin(void)
{
	...
	{
		GetToken(false);
		defaultzrotation = atof(token) * (Q_PI / 180.0);
	}
}
...
{
	GetToken(false);
	zrotation = atof(token) * (Q_PI / 180.0);
}
...
{
	GetToken(false);
	printf("WARNING: $sequencegroup \"%s\" ignored (HL Alpha writes a single embedded MDL).\n", token);
...
{
	GetToken(false);
	printf("WARNING: $sequencegroupsize ignored in HL Alpha mode.\n");
	return 0;
}
...
int Cmd_Sequence()
{
	...
	if (numblends > 1)
	{
		printf("WARNING: sequence \"%s\" has %d blends; HL Alpha keeps the first animation only.\n",
...
int main(int argc, char** argv)
{
	...
	if (argc == 1)
		Error("usage: studiomdl [-t texture] -r(tag reversed) -n(tag bad normals) -f(flip all triangles) [-a normal_blend_angle] -h(dump hboxes) -i(ignore warnings) -p(force power of 2 textures) file.qc");
	...
	for (i = 1; i < argc - 1; i++)
	{
		if (argv[i][0] == '-')
		{
			...
			case 'g':
				i++;
				printf("WARNING: -g ignored in HL Alpha mode.\n");
				break;
			case 'p':
			case '2':
